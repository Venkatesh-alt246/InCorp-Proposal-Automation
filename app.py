...
def build_pdf_elements(data):
    ...
    firstLineIndent=-15,  # ← bullet 10pt peeche jaayega (hanging indent)
    rightIndent=0,
    spaceAfter=6,
)
...
def generate_proposal_word():
    ...
    try:
        ...
        def _make_pPr(align='left', space_before=0, space_after=0, left_indent=0, first_line=0):
            ...
            if first_line < 0:
                  ind.set(qn('w:hanging'), str(abs(int(first_line * 20))))
            elif first_line > 0:
        ...
        def _bul(text, size_pt=10, color_hex='777777'):
            ...
            return _p(f'\u2022 {clean}', size_pt=size_pt, color_hex=color_hex, font='Roboto', align='left', li=10, fi=-10, sb=0, sa=3)
        def _bul(text, size_pt=10, color_hex='777777'):
            clean = text.lstrip('•').lstrip('\u2022').strip()
            return _p(f'\u2022 {clean}', size_pt=size_pt, color_hex=color_hex, font='Roboto', align='left', li=10, fi=-10, sb=0, sa=3)
        def _bul11(text):
            clean = text.lstrip('•').lstrip('\u2022').strip()
            return _p(f'\u2022 {clean}', size_pt=11, font='Roboto', align='left', li=11, fi=-11, sb=0, sa=3)
        ...
        def _note_block(items):
            ...
            out.append(_sp(0))
            for item in items:
                out.append(_p(f'\u2022 {item}', size_pt=10, color_hex='444444',
                    font='Roboto', align='left', li=10, fi=-10, sa=2))
            out.append(_sp(1))
            return out
